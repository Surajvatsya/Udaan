...
const RestaurantDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchHeatmapData = async () => {
      try {
        const response = await fetch(`http://localhost:3000/api/order/heatmap/${id}`);
        if (!response.ok) {
  ...
  const fetchTimelineData = async () => {
    try {
      const response = await fetch(`http://localhost:3000/api/interactions/${id}`);
      if (!response.ok) {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
          ...
          <div className="flex justify-between w-full mt-2">
            <div className="flex items-center space-x-2">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 11c.83 0 1.5-.67 1.5-1.5S12.83 8 12 8s-1.5.67-1.5 1.5S11.17 11 12 11zm0 0c-1.33 0-4 .67-4 2v1h8v-1c0-1.33-2.67-2-4-2zm0 4v3" />
              </svg>
              <p>{restaurant.location}</p>
            </div>
            <div className="flex items-center space-x-2">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18v-4a4 4 0 014-4h4a4 4 0 014 4v4M8 8a4 4 0 118 0m-8 0h8" />
              </svg>
      ...
      <div className="m-6 flex bg-white shadow rounded-md">
        <button
          className={`flex-1 py-3 text-center ${activeTab === 'Timeline' ? 'bg-blue-600 text-white' : 'text-gray-600'
            }`}
        ...
        <button
          className={`flex-1 py-3 text-center ${activeTab === 'Orders' ? 'bg-blue-600 text-white' : 'text-gray-600'
            }`}
        ...
        <button
          className={`flex-1 py-3 text-center ${activeTab === 'POC' ? 'bg-blue-600 text-white' : 'text-gray-600'
            }`}
            ...
            <tbody>
              {orderData.map((order) => (
                <tr key={order.order_id} className="border-b">
                  ...
                  <td className="py-2 text-green-600">{order.order_status}</td>
                  <td className="py-2">
                    <button
...
{activeTab === 'POC' && (
          <div>
            <h2 className="text-xl font-semibold text-gray-800">Point of Contacts</h2>
